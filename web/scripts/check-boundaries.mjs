...
for (const required of ['AuthMiniProvider', 'autoRedirectToLogin', 'LinkitProvider', 'LinkitMyInfo', 'LinkitUserPicker', 'QRCodeSVG', 'qrcode.react', 'QueryClientProvider', 'HashRouter', '<Toaster position="top-center" closeButton />', '/api/admin/deposit-discovery', '/api/admin/fund-users', 'fundUsers', 'fundUserApiKey', 'fundUserId', 'copyUserId', 'userIdCopied', 'navigator.clipboard.writeText(fund.id)', '/api/deposits/claim', 'depositDiscovery', '/api/transfers', '/api/withdrawals', '/api/withdrawal-targets/me', '/api/agreements/owned', '/api/agreements/bindings/me', '/api-key', '/sign_agreement', '/transfer', 'TransferCheckoutPage', 'useSearchParams', 'destination_address', 'Idempotency-Key', 'automaticChargeHelpTitle', '$MIDAS_API_KEY', 'amount_usd_nanos']) if (!source.includes(required)) throw new Error(`Missing Midas integration: ${required}`);
for (const forbidden of ['FundUserWithdrawalDrawer', 'fundUserAddress', 'fundUserWithdrawalBody', 'fund.wallet_address']) if (source.includes(forbidden)) throw new Error(`Fund users must not expose EVM operations: ${forbidden}`);
