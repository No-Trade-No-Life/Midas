...
const messages = {
  en: {
    ...
    balance: "Available balance", usdOnly: "USD ledger", deposit: "Deposit", transfer: "Transfer", withdraw: "Withdraw", depositAddress: "Your deposit address", depositAddressHint: "This single EVM-compatible address works on every supported network.", copy: "Copy", copied: "Address copied", noAddress: "Your dedicated EVM address is being prepared. Refresh shortly.",
  ...
  zh: {
    ...
    balance: "可用余额", usdOnly: "USD 账本", deposit: "充值", transfer: "转账", withdraw: "提现", depositAddress: "你的充值地址", depositAddressHint: "同一个 EVM 兼容地址可用于所有已支持的网络。", copy: "复制", copied: "地址已复制", noAddress: "专属 EVM 地址正在准备，请稍后刷新。",
...
function WithdrawalRow({ withdrawal, t }: { withdrawal: Withdrawal; t: Translate }) {
  ...
  const saveDestination = useMutation({ mutationFn: () => api<AddressBookEntry>(`/api/withdrawals/${withdrawal.id}/address-book`, { method: "POST", body: { label } }), onSuccess: () => { toast.success(t("addressAdded")); setLabel(""); void queryClient.invalidateQueries({ queryKey: ["withdrawals"] }); void queryClient.invalidateQueries({ queryKey: ["address-book"] }) }, onError: () => toast.error(t("apiError")) })
